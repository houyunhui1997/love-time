...
用户后续主动打开资料弹窗时，`profile-co.saveCompleteProfile` 一次完成全部资料：
...
资料弹窗保存后更新 `uni-id-users` 中的昵称、头像和性别，并首次创建或更新 `love-profiles` 中的在一起日期与可选对方称呼。系统不保存对方头像；这份档案仅归当前账号所有，不完善资料也可以正常使用纪念日与时光记录。
...
4. 在资料弹窗中保存后，`love-profiles` 应新增一条记录，首页按填写日期计算相伴天数。
