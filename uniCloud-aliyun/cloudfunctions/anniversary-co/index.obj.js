...
module.exports = {
  ...
  async detail(params = {}) {
    try {
      ...
      if (item.creatorUid !== ownerUid) throw new AppError(API_CODE.FORBIDDEN, '无权访问该纪念日')
  ...
  async update(params = {}) {
    try {
      ...
      if (existing.creatorUid !== ownerUid) throw new AppError(API_CODE.FORBIDDEN, '无权修改该纪念日')
      ...
      const updated = await anniversaries.where({ _id: id, creatorUid: ownerUid, revision, status: 'active', 'subscription.status': dbCmd.neq('sending') }).update({ ...patch, reminderVersion: 2, reminderOffsetDays: nextOffsets, ...subscriptionPatch, updatedByUid: auth.uid, updatedAt: Date.now(), revision: dbCmd.inc(1) })
  ...
  async remove(params = {}) {
    try {
      ...
      const existing = existingResult.data && existingResult.data[0]
      if (!existing || existing.status !== 'active') throw new AppError(API_CODE.NOT_FOUND, '纪念日不存在')
      if (existing.creatorUid !== ownerUid) throw new AppError(API_CODE.FORBIDDEN, '无权删除该纪念日')
      const removed = await anniversaries.where({ _id: id, creatorUid: ownerUid, revision: existing.revision, 'subscription.status': dbCmd.neq('sending') }).update({ subscription: { status: 'cancelled' }, status: 'deleted', deletedAt: Date.now(), updatedByUid: auth.uid, updatedAt: Date.now(), revision: dbCmd.inc(1) })
